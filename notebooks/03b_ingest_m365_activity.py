...
def fetch_activity_report(config, token):
    base_url = config["graph_api"]["base_url"].rstrip("/")
    next_link = (
        f"{base_url}/v1.0/reports/getOffice365ActiveUserDetail"
...
def write_bronze_activity(records, watermark):
        ...
        output.append(
            {
                ...
                "exchangeLastActivityDate": item.get("exchangeLastActivityDate"),
                "oneDriveLastActivityDate": item.get("oneDriveLastActivityDate"),
                "sharePointLastActivityDate": item.get("sharePointLastActivityDate"),
                "teamsLastActivityDate": item.get("teamsLastActivityDate"),
                "wordLastActivityDate": item.get("wordLastActivityDate"),
                "excelLastActivityDate": item.get("excelLastActivityDate"),
                "powerPointLastActivityDate": item.get("powerPointLastActivityDate"),
                "_ingested_at": ingested_at,
                "_source_api": "/v1.0/reports/getOffice365ActiveUserDetail(period='D30')",
                "_batch_id": batch_id,
            }
        )
    ...
    (
        ...
        .mode("append")
        .saveAsTable("bronze_m365_activity")
    )
...
def main():
    ...
    watermark = get_last_report_refresh_date("m365_activity")
    records = fetch_activity_report(config, token)
    count, latest_report_refresh_date = write_bronze_activity(records, watermark)
    upsert_control_state("m365_activity", count, "SUCCESS", latest_report_refresh_date)
